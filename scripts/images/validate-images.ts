import { extractImageReferences, pathExists, printResult, readContentEntries, readText, walkFiles, type AuditMessage } from '../shared/toolkit.ts'

const messages: AuditMessage[] = []
const scanned = [
  ...(await readContentEntries()).map((entry) => ({ file: entry.relativePath, body: `${entry.content}\n${JSON.stringify(entry.data)}` })),
  ...walkFiles('app', ['.tsx', '.ts']).map((file) => ({ file, body: readText(file) })),
  ...walkFiles('components', ['.tsx', '.ts']).map((file) => ({ file, body: readText(file) })),
]
