...
export function writeConsentState(consent: Omit<ConsentState, 'updatedAt'>): ConsentState {
  const state = { ...consent, updatedAt: new Date().toISOString() }

  if (typeof window !== 'undefined') {
    window.localStorage.setItem(analyticsConfig.consentStorageKey, JSON.stringify(state))
...
export function applyConsent(consent: Pick<ConsentState, 'analytics' | 'marketing'>) {
  if (typeof window === 'undefined') return
...
export function saveConsent(consent: Pick<ConsentState, 'analytics' | 'marketing'>): ConsentState {
